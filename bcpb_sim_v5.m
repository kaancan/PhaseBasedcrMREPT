%% Bias-corrected standart and cr-MREPT -- 12.2025 -- v5
...
% Boundary Condition
opts.boundaryCondition      = 0;
opts.boundaryConditionValue = 0.5;
...
bc_out = 1/opts.boundaryConditionValue;
...
if(opts.plots)
    figure
    pdesurfk(ROI.masked.pp,ROI.masked.tt,recon.cond.std');colormap jet;
    ...
    figure
    pdesurfk([ROI.masked.x; ROI.masked.y],ROI.masked.tt,recon.cond.cr+0.06);colormap jet;
    colormap jet; clim([0 1]);%zlim([0 1]);%%ylim([min(ROI.masked.y) max(ROI.masked.y)]);xlim([min(ROI.masked.x) max(ROI.masked.x)]);
    title('cr-MREPT')

    if(opts.boundaryCondition)
    figure
    pdesurfk([ROI.masked.x; ROI.masked.y],ROI.masked.tt,local.cond.crBC);colormap jet;
    colormap jet; clim([0 1]);%zlim([0 1]);%%ylim([min(ROI.masked.y) max(ROI.masked.y)]);xlim([min(ROI.masked.x) max(ROI.masked.x)]);
    title('cr-MREPT - with BC')
    end
end